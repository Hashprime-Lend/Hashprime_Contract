...
abstract contract HToken is HTokenInterface, Exponential, TokenErrorReporter {
    uint256 public borrowFeeMantissa;
        ...
        returns (uint256)
    {
        ...
        if (redeemTokensIn > 0) {
            ...
            if (vars.mathErr != MathError.NO_ERROR) {
                return failOpaque(
                    Error.MATH_ERROR, FailureInfo.REDEEM_EXCHANGE_AMOUNT_IN_CALCULATION_FAILED, uint256(vars.mathErr)
                );
            }
    ...
    function borrowFresh(address payable borrower, uint256 borrowAmount) internal returns (uint256) {
        ...
        uint256 fee = borrowAmount * borrowFeeMantissa / 1e18;

        if (fee > 0) {
            _increaseReserveFresh(fee);
        }

        /* Deduct borrow fee from the borrowAmount */
        uint256 netBorrowAmount = borrowAmount - fee;
         ...
         *  doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
         */
        doTransferOut(borrower, netBorrowAmount);
    ...
    /**
     * @notice Increase reserves by fee
     * @dev Requires fresh interest accrual
     * @param addAmount Amount of addition to reserves
     * @return (uint, uint) An error code (0=success, otherwise a failure (see ErrorReporter.sol for details)) and the actual amount added, net token fees
     */
    function _increaseReserveFresh(uint256 addAmount) internal returns (uint256, uint256) {
        // totalReserves + actualAddAmount
        uint256 totalReservesNew;
        uint256 actualAddAmount;
        ...
        totalReservesNew = totalReserves + addAmount;
    ...
    /**
     * @notice updates the protocol borrow fee
     * @dev Admin function to update the protocol seize share
     * @param _borrowFeeMantissa the borrow fee percentage when user borrow
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setBorrowFee(uint256 _borrowFeeMantissa) external returns (uint256) {
        uint256 oldBorrowFeeMantissa;

        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_PROTOCOL_BORROW_FEE_CHECK);
        }

        oldBorrowFeeMantissa = borrowFeeMantissa;
        borrowFeeMantissa = _borrowFeeMantissa;

        emit NewBorrowFee(oldBorrowFeeMantissa, borrowFeeMantissa);
