    ...
    ExponentialNoError
{
    ...
    uint256 public refereeBonusPercentage; // Initial  0.5%

    // Mapping to store referral data for each user
    ...
    */
    ...
    function getAccountReferralInfo(address _account) external view returns (ReferralData memory) {
        return referralInfo[_account];
    }
    ...
    */
    ...
    function _setRefereeBonus(uint256 percentage) external onlyComptrollersAdmin {
        refereeBonusPercentage = percentage;
    }

    // Set a user's referrer
    function _setReferrer(address referrer) public {
        require(referralInfo[msg.sender].referrer == address(0), "Already bound");
        require(referrer != msg.sender, "Cannot refer yourself");
        referralInfo[msg.sender].referrer = referrer;
        referralInfo[referrer].referees.push(msg.sender);
    }
    ...
    */
        ...
        returns (uint256)
    {
        ...
        // Get the user's referral info
        ReferralData memory userReferralInfo = referralInfo[_user];

        uint256 distributeRewards = _amount;
        uint256 reward;

        if (userReferralInfo.referrer != address(0)) {
            // Calculate the reward
            reward = _amount * refereeBonusPercentage / 1e18;
        ...
        if (distributeRewards > 0 && distributeRewards <= currentTokenHoldings) {
            // Ensure we use SafeERC20 to revert even if the reward token isn't ERC20 compliant
            token.safeTransfer(_user, _amount);

            if (reward > 0) {
                token.safeTransfer(userReferralInfo.referrer, reward);
            }

            referralInfo[userReferralInfo.referrer].rewards += reward;

            emit RewardDistributed(_rewardToken, _user, userReferralInfo.referrer, _amount, reward);
