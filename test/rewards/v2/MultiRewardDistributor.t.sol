...
import "src/upgrades/rewards/MultiRewardDistributor.sol";
import "src/upgrades/rewards/IMultiRewardDistributor.sol";
...
contract MultiRewardBorrowSideDistributorUnitTest is Test, ExponentialNoError, Common {
    ...
    function testBorrowerHappyPath() public {
        ...
        comptroller._setRewardDistributor(IMultiRewardDistributorV1(address(distributor)));
...
contract MultiRewardSupplySideDistributorUnitTest is Test, ExponentialNoError, Common {
    ...
    function testMultipleEmissionTokens() public {
        ...
        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(address(distributor), proxyAdmin, initdata);
        /// wire proxy up
        distributor = MultiRewardDistributor(payable(address(proxy)));
...
contract MultiRewardDistributorCommonUnitTest is Test, ExponentialNoError, Common {
    ...
    function testBorrowSupplyRewardsWithReferralInfoFuzz(
        uint256 rewardsPerSecondBorrow,
        uint256 rewardsPerSecondSupply,
        uint256 secondsToWarp
    ) public {
        address referrer = makeAddr("referrer");
        uint256 refereeBonusPercentage = 0.005e18;

        distributor._setRefereeBonus(refereeBonusPercentage);
        distributor._setReferrer(referrer);
        ...
        assertEq(emissionToken.balanceOf(address(this)), 0);
        assertEq(emissionToken.balanceOf(referrer), 0);
        ...
        vm.expectEmit(true, true, true, false, address(distributor));
        emit RewardDistributed(
            rewardInfo[0].emissionToken,
            address(this),
            referrer,
            rewardInfo[0].totalAmount,
            rewardInfo[0].totalAmount * refereeBonusPercentage / 1e18
        );
        comptroller.claimReward();
        ...
        assertApproxEqRel(
            emissionToken.balanceOf(referrer),
            ((rewardsPerSecondBorrow * secondsToWarp) + (rewardsPerSecondSupply * secondsToWarp)) * 0.005e18 / 1e18,
            0.00001e18
        );
    }
}
